...
async def testing(loop):
    ...
    async with aiohttp.ClientSession(connector=aiohttp.TCPConnector(verify_ssl=False)) as session:
        ...
        blob[PC] = await client.get_full_profile("Danielfrogs#2552", session=session, platform=PC)
        print("lmao")
        # We sleep to avoid getting a owapi_overwatch_api.exceptions.RatelimitError (http 429 code from the api)
        await asyncio.sleep(5)
        blob[XBOX] = await client.get_full_profile("Danielfrogs#2552", session=session, platform=XBOX)
        print("lmao")
        await asyncio.sleep(5)
        blob[PLAYSTATION] = await client.get_full_profile("Danielfrogs#2552", session=session, platform=PLAYSTATION)
    print(blob)
